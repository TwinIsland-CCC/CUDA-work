...
__global__ void div_kernel(float* data, int k, int N) {
	...
	//答：为防止小于n，应该使用循环让一个线程多执行几个数，同时因为是一维数组，不太容易越界报异常，所以一定要非常仔细地检查下标
	while (k + tid + 1 < N) {
...
float* generate_1d(int n) {
	...
	for (int i = 0; i < n; i++) for (int j = 0; j < n; j++) inp >> m[i * n + j];
	inp.close();
	return m;
}
...
void show_1d(float* a, int n) {  // 用于观察程序运行结果
	for (int i = 0; i < n; i++)
	{
...
void show_in_file_1d(float* a, int n) {  // 用于观察程序运行结果
	ofstream out("output.txt");
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < n; j++) out << a[i * n + j] << " ";
		out << endl;
	}
	out.close();
}
